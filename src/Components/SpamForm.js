...
const SpamForm = () => {
    ...
    const [formData, setFormData] = useState({
      ...
      const validate = (inputName = '') => {
        ...
        if (formData.headers.length < 3) {
          error.headers = 'headers should be at least 3 characters';
        }
        if (formData.messageBody.length < 3) {
          error.messageBody = 'message Body should be at least 3 characters';
        }
        if (formData.digitalSignature.length < 3) {
          error.digitalSignature = 'Signature should be at least 3 characters';
        }
        if (formData.urls.length < 30) {
          error.urls = 'URLs content should be at least 30 characters';
        }
            ...
            <div className="row">
                ...
                <div className="col-lg-6">
                    ...
                    <label className="form-label" htmlFor="headers">
                        <span className="input-title">Headers from email received</span>
                        <textarea className={`form-textarea ${(errors.headers)? "error" : ((formData.headers != '')?'valid':'')}`} name="headers" id="headers" required
                        value={formData.headers}
                        // onChange={event => setheaders(event.target.value)}
                        onChange={handleChange}
                        onBlur={handleBlur}
                        />
                        {errors.headers && <span className="error">{errors.headers}</span>}
                        <span className="notify notify-valid"><img src="/assets/images/pages/abuse/fi-sr-check.svg" className="lazy img-fluid" alt="Icon" /></span>
                        <span className="notify notify-error"><img src="/assets/images/pages/abuse/fi-sr-cross.svg" className="lazy img-fluid" alt="Icon" /></span>
                    </label>
                </div>
                ...
                <div className="col-lg-6">
                    ...
                    <label className="form-label" htmlFor="messageBody">
                        <span className="input-title">Message Body</span>
                        <textarea className={`form-textarea ${(errors.messageBody)? "error" : ((formData.messageBody != '')?'valid':'')}`} name="messageBody" id="messageBody" required
                ...
                <div className="col-lg-12">
                    ...
                    <label className="form-label" htmlFor="urls">
                        <span className="input-title">Spamvertisement URLs</span>
